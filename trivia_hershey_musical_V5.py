...
class HersheysTriviaApp:
    ...
    def __init__(self, root):
        ...
        try:
            self.sonido_boton = pygame.mixer.Sound("nice-celest.mp3")  # Cambia por el nombre de tu archivo de audio
            self.sonido_boton.set_volume(0.8)
        except Exception as e:
            print(f"No se pudo cargar el sonido del botón: {e}")
            self.sonido_boton = None

        self.ruta_musica_inicio = obtener_ruta_recurso(os.path.join("audio", "musica_inicio.mp3"))
        self.ruta_musica_juego = obtener_ruta_recurso(os.path.join("audio", "musica_juego.mp3"))
        self.ruta_musica_registro = obtener_ruta_recurso(os.path.join("audio", "musica_registro.mp3"))
        self.ruta_musica_resultado = obtener_ruta_recurso(os.path.join("audio", "musica_resultado.mp3"))
        ...
        # --- TIMER DE INACTIVIDAD/AUTO-REINICIO EN REGISTRO ---
        self.timer_registro_id = None
        self.tiempo_restante_registro = 15  # Segundos de espera antes de reiniciar
        ...
        try:
            self.img_boton_raw = Image.open("BotonesH.jpg")
        except Exception as e:
            print(f"Error al cargar BotonesH.jpg: {e}")
            self.img_boton_raw = None

        # Lista para mantener referencias vivas a las imágenes renderizadas
        ...
        self.preguntas = [
            ...
            },
            {
                ...
                "opciones": [
                    ...
                    ("2. Hershey's Cookies 'n' Cream", 20, "images/cookies.jpg"),
                    ("3. Hershey's Special Dark", 15, "images/dark.jpg"),
                    ("4. Todos por su olor característico", 25, "images/variedad.jpg")
                ]
            },
            {
                "id": 3,
                ...
                "opciones": [
                    ...
                    ("3. Reese's Peanut Butter Cups", 20, "images/reeses.jpg"),
                    ("4. ¡Todos pertenecen a la familia Hershey's!", 25, "images/variedad.jpg")
                ]
            },
            {
                "id": 4,
            ...
            },
            {
                ...
                "opciones": [
                    ...
                    ("4. Vivo para comer chocolate", 25, "images/variedad.jpg")
                ]
            }
        ]
    ...
    def cancelar_timer_registro(self):
        """Cancela el timer del registro si está activo."""
    ...
    def cargar_logo(self):
        ...
        ruta_logo_blanco = obtener_ruta_recurso(os.path.join("images", "LogoBlanco.png"))
        ruta_logo_marron = obtener_ruta_recurso(os.path.join("images", "LogoMarron.png"))
    ...
    def limpiar_pantalla(self):
        """Limpia widgets, desvincula eventos de la botonera y cancela timers."""
        ...
        # Desvincular teclas de la botonera
        for tecla in ["1", "2", "3", "4", "<KP_1>", "<KP_2>", "<KP_3>", "<KP_4>"]:
            try:
    ...
    def cargar_fondos(self):
        """Carga y escala ambas imágenes de fondo."""
    ...
    def salir_juego(self):
        # Opcional: Detener música si tienes un método para ello
        if hasattr(self, 'detener_musica'):
            try:
    ...
    def mostrar_pantalla_inicio(self):
        ...
        # --- AHORA CON BORDES REDONDEADOS (radio=20) ---
        btn_shadow = self.crear_rectangulo_redondeado(
            main_canvas, x1_btn + 4, y1_btn + 4, x2_btn + 4, y2_btn + 4, 
            radio=20, fill="#120704", outline=""
        )
        btn_rect = self.crear_rectangulo_redondeado(
            main_canvas, x1_btn, y1_btn, x2_btn, y2_btn, 
            radio=20, fill=GOLD_ACCENT, outline="#FFFFFF", width=2
        )
        ...
        # Iniciar trivia con cualquier botón físico (1-4)
        for tecla in ["1", "2", "3", "4", "<KP_1>", "<KP_2>", "<KP_3>", "<KP_4>"]:
        ...
        if hasattr(self, 'logo_blanco_tk') and self.logo_blanco_tk:
            main_canvas.create_image(
                centro_x, alto_pantalla - 100, image=self.logo_blanco_tk, anchor="center"
            )
    ...
    # ==========================================
    # PANTALLA 2: PREGUNTAS (SIN RECUADRO DE FOTOS)
    # ==========================================
    def mostrar_pregunta(self):
        ...
        # Logo en lugar del texto placeholder
        ...
        # Renderizar opciones (botones de chocolate con BotonesH.jpg)
        y_inicial = 290
        espaciado = 85
        btn_ancho = 550
        btn_alto = 70

        # Limpiar lista de imágenes anteriores para evitar fugas de memoria
        if not hasattr(self, 'imagenes_botones'):
            self.imagenes_botones = []
        self.imagenes_botones.clear()

        for i, (texto, puntos, foto_path) in enumerate(q_data["opciones"]):
            pos_y = y_inicial + (i * espaciado)
            texto_completo = f"{i + 1}. {texto}"

            # Si la imagen base BotonesH.jpg existe, la redimensionamos y dibujamos
            if hasattr(self, 'img_boton_raw') and self.img_boton_raw:
                img_resized = self.img_boton_raw.resize((btn_ancho, btn_alto), Image.Resampling.LANCZOS)
                tk_img = ImageTk.PhotoImage(img_resized)
                self.imagenes_botones.append(tk_img)  # Mantiene la referencia viva

                btn_img_id = self.main_canvas.create_image(
                    centro_x, pos_y, image=tk_img, anchor="center"
                )
            else:
                # Botón de respaldo por si no se carga la imagen
                btn_img_id = self.main_canvas.create_rectangle(
                    centro_x - (btn_ancho // 2), pos_y - (btn_alto // 2),
                    centro_x + (btn_ancho // 2), pos_y + (btn_alto // 2),
                    fill="#3d1c11", outline="#ffffff", width=2
                )

            # Texto centrado sobre la tableta de chocolate
            ...
            # Clic para pantalla táctil o mouse
        ...
        # Vincular botonera física (teclado / keypad)
        self.root.bind("1", lambda e: self.seleccionar_opcion_por_indice(0))
        self.root.bind("2", lambda e: self.seleccionar_opcion_por_indice(1))
        self.root.bind("3", lambda e: self.seleccionar_opcion_por_indice(2))
        self.root.bind("4", lambda e: self.seleccionar_opcion_por_indice(3))

        self.root.bind("<KP_1>", lambda e: self.seleccionar_opcion_por_indice(0))
        self.root.bind("<KP_2>", lambda e: self.seleccionar_opcion_por_indice(1))
        self.root.bind("<KP_3>", lambda e: self.seleccionar_opcion_por_indice(2))
        self.root.bind("<KP_4>", lambda e: self.seleccionar_opcion_por_indice(3))

    def seleccionar_opcion_por_indice(self, index):
        q_data = self.preguntas[self.pregunta_actual]
        ...
        if 0 <= index < len(opciones):
            ...
            # 1. Determinar cuál posición contiene los puntos y cuál el texto
            if isinstance(opcion, (tuple, list)):
                elem1, elem2 = opcion[0], opcion[1]
                
                # Si el primer elemento es el texto y el segundo los puntos: ("Texto", 10)
                if str(elem1).replace('-', '').isdigit() is False and str(elem2).replace('-', '').isdigit() is True:
                    texto, puntos_raw = elem1, elem2
                else:
                    # Estructura habitual: (puntos, "Texto", ...)
                    puntos_raw, texto = elem1, elem2
            else:
                puntos_raw, texto = 0, str(opcion)

            # 2. Convertir puntos a entero de forma segura
            try:
                puntos = int(puntos_raw)
            except (ValueError, TypeError):
                puntos = 0

            # 3. Sumar al puntaje y registrar respuesta
            self.score += puntos
            self.respuestas_usuario[f"pregunta_{self.pregunta_actual + 1}"] = texto

            # Desvincular entradas del teclado
    ...
    def siguiente_pregunta(self):
        ...
        if self.pregunta_actual < len(self.preguntas):
            ...
            # Fin de las preguntas -> Detener música de juego e ir al registro
    ...
    def crear_boton_chocolate_canvas(self, canvas_padre, x, y, index, texto, puntos, foto_path):
        ...
        def seleccionar_opcion(e):
            self.seleccionar_opcion_por_indice(index)
    ...
    # ==========================================
    # PANTALLA 3: REGISTRO (CON TIMER DE INACTIVIDAD)
    # ==========================================
    def mostrar_formulario_registro(self):
        self.limpiar_pantalla()
        self.reproducir_musica_registro()

        # Tiempo límite antes de auto-reiniciar (puedes cambiarlo a 10, 20 o 30 seg)
        ...
        # Label con el temporizador visible
        ...
        card_form = tk.Frame(center_frame, bg=BG_DARK_HEADER, padx=35, pady=30, highlightbackground=GOLD_ACCENT, highlightthickness=2)
        ...
        # Si el usuario empieza a tipear, reiniciamos la cuenta regresiva
    ...
    def reiniciar_timer_registro(self, event=None):
        """Si el usuario escribe algo, le damos tiempo completo nuevamente."""
        self.tiempo_restante_registro = 15
        if hasattr(self, 'lbl_timer_registro') and self.lbl_timer_registro.winfo_exists():
            self.lbl_timer_registro.config(
                text=f"⏱️ Tiempo restante: {self.tiempo_restante_registro}s"
            )

    def actualizar_timer_registro(self):
        """Resta 1 segundo a la cuenta regresiva. Al llegar a 0, salta a inicio."""
        if self.tiempo_restante_registro > 0:
            ...
            self.timer_registro_id = self.root.after(1000, self.actualizar_timer_registro)
        else:
    ...
    def mostrar_resultado(self, perfil):
        ...
        card_res.pack(pady=15)

        lbl_titulo_perfil = tk.Label(
            card_res,
            text="TU NIVEL ES:",
            font=("Helvetica", 16),
            bg=BG_DARK_HEADER,
        ...
        lbl_perfil.pack(pady=10)

        lbl_score = tk.Label(
            card_res,
            text=f"Puntuación final: {self.score} pts",
            font=("Helvetica", 16),
            bg=BG_DARK_HEADER,
            fg=TEXT_DARK_WHITE
        )
        ...
        for tecla in ["1", "2", "3", "4", "<KP_1>", "<KP_2>", "<KP_3>", "<KP_4>"]:
            self.root.bind(tecla, lambda e: self.mostrar_pantalla_inicio())
